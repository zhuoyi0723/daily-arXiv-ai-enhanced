...
TL;DR: 提出一个缓解开放集识别中过度自信问题的框架，通过基于扰动的不确定性估计和两阶段未知检测模块来提升已知/未知类别区分性能。


<details>
  <summary>Details</summary>
Motivation: 开放集识别中，当未知样本与已知类别语义相似时，特征空间的类间重叠会导致模型对未知样本过度自信，错误地将其分类为已知类别，破坏了已知/未知类别的决策边界。

Method: 框架包含两个组件：1) 基于扰动的不确定性估计模块，通过可控参数扰动生成多样化预测并量化预测不确定性；2) 基于学习的未知检测模块，采用两阶段过程，利用估计的不确定性改善已知/未知类别的区分。

Result: 在三个公共数据集上的实验结果表明，提出的框架在开放集识别任务中取得了优于现有方法的性能。

Conclusion: 通过显式缓解类间重叠导致的过度自信问题，结合不确定性估计和两阶段未知检测，有效提升了开放集识别的准确性和鲁棒性。
...
TL;DR: 提出一种基于对象感知时间建模的少样本视频目标检测方法，通过选择性传播高置信度目标特征实现更好的检测性能。


<details>
  <summary>Details</summary>
Motivation: 传统目标检测方法需要大量训练数据，少样本视频目标检测面临时间一致性维护、遮挡处理、外观变化等挑战，且现有方法依赖计算昂贵的复杂区域提议机制。

Method: 提出对象感知的时间建模方法，包含过滤机制来选择性传播高置信度目标特征，结合少样本训练的检测和分类头，实现高效的特征传播和噪声减少。

Result: 在5-shot设置下，多个数据集获得显著性能提升：FSVOD-500提升3.7%、FSYTV-40提升5.3%、VidOR提升4.3%、VidVRD提升4.5%的AP，在1-shot、3-shot、10-shot配置下也表现优异。

Conclusion: 该方法无需依赖显式对象管提议即可实现鲁棒的时间一致性，为少样本视频目标检测提供了有效解决方案，代码已公开供研究使用。
...
TL;DR: 本文提出了一种基于流匹配(flow matching)的多模态图像融合统一框架，通过直接概率传输、伪标签选择与精炼、以及持续学习机制，实现了高效的多任务图像融合。


<details>
  <summary>Details</summary>
Motivation: 现有多模态图像融合方法存在两大问题：1) 任务特定模型导致训练成本高、可扩展性差；2) 生成式方法提供统一建模视角但推理速度慢，因为需要从噪声到图像的复杂采样轨迹。需要一种既能统一建模又高效的方法。

Method: 1) 将图像融合构建为从源模态到融合图像分布的直接概率传输，采用流匹配范式提升采样效率和结构一致性；2) 收集多个SOTA模型的融合结果作为先验，使用任务感知选择函数为每个任务选择最可靠的伪标签；3) 引入融合精炼模块，采用分治策略系统识别、分解和增强伪标签中的退化组件；4) 对于多任务场景，集成弹性权重合并和经验回放机制，从参数稳定性和记忆保持角度保持跨任务性能。

Result: 该方法在多样化融合任务上实现了竞争性能，同时显著提高了采样效率并保持了轻量级的模型设计。

Conclusion: 本文成功地将流匹配范式引入图像融合领域，通过创新的伪标签管理和持续学习策略，解决了传统方法在训练成本、推理效率和跨任务适应性方面的限制，为多模态图像融合提供了一个高效统一的解决方案。
...
TL;DR: 提出了一种新颖的两阶段无轨迹碰撞检测框架，使用道路段地图进行实时碰撞检测


<details>
  <summary>Details</summary>
Motivation: 实时碰撞检测对开发主动安全管理策略和提高整体交通效率至关重要，但传统轨迹获取和车辆跟踪方法存在局限性

Method: 设计两阶段框架：第一阶段使用基于扩散的段地图生成模型Mapfusion，通过噪声到正常过程逐步添加噪声直至纯高斯噪声，去噪过程由捕获时序动态的序列嵌入组件引导，并通过ControlNet集成背景上下文增强生成控制；第二阶段通过比较监控段地图与扩散模型生成的地图进行碰撞检测

Result: Mapfusion基于非碰撞车辆运动数据训练，能根据学习到的运动模式成功生成逼真的道路段演化地图，在不同采样间隔下保持鲁棒性；真实碰撞实验表明该方法能准确检测碰撞

Conclusion: 所提出的两阶段无轨迹碰撞检测方法有效解决了传统轨迹获取和跟踪的局限性，在实时碰撞检测方面表现出良好的准确性和实用性
...
TL;DR: 本文提出ADATG，一种结合希尔伯特编码的自适应双网格训练策略，专门针对地震数据的高低频特征进行优化，有效提升地震基础模型的预训练效果。


<details>
  <summary>Details</summary>
Motivation: 现有视觉Transformer模型在处理地震数据时存在缺陷：序列化tokenization方法忽略了地震数据的内在模式，无法同时有效捕获高、低频地震信息，而这两个频段信息在地震分析中都起着关键作用。

Method: 提出自适应双网格基础模型训练策略(ADATG)：1)使用频谱分解分离高、低频分量；2)采用层次化希尔伯特编码有效表示数据；3)基于ViT频率原理设计自适应训练策略，先关注粗粒度信息，再逐步细化到细粒度特征。

Result: 广泛的实验结果表明，所提出的训练方法在效果和效率上都表现出色，能够有效处理地震数据的高低频特征。

Conclusion: 本研究强调了基于地震数据高低频特征独特特性的数据编码和训练策略的重要性，为增强视觉地震基础模型的预训练提供了新的思路和方法。
...
TL;DR: 本文开发了一种基于面部时间微动力学的被动式痴呆症筛查方法，通过分析短视频中的人脸行为特征，无需语言或文本即可实现早期神经认知变化的检测。


<details>
  <summary>Details</summary>
Motivation: 现有痴呆症筛查资源主要依赖语音或脚本化访谈，限制了临床外的应用并使预测与语言和转录相关联。需要开发一种无需临床医生主动干预、可在自然环境下大规模分析无脚本视频的方法。

Method: 通过分析和稳定面部信号，将眨眼动态、嘴部下颌微小运动、注视变异性、头部细微调整等微动特征转换为可解释的面部微动态时间序列，然后进行平滑处理并将短窗口总结为紧凑的片段级统计数据。每个窗口通过其活动混合（不同运动流的相对份额）进行编码，使预测器分析运动分布而非幅度，提高透明度。

Result: 在包含300个片段的新数据集YT DemTalk上，该方法实现了AUROC 0.953、平均精度0.961、F1分数0.851和准确率0.857的性能。消融实验表明注视不稳定性和嘴部/下颌动态是最具信息量的线索。

Conclusion: 面部时间微动力学分析足以进行无需语音或文本的痴呆症筛查，为跨设备、主题和文化的自然环境下大规模痴呆症检测提供了有效方法。
...
TL;DR: Gen-ViRe是一个用于评估视频生成模型视觉推理能力的基准框架，它将视觉推理分解为6个认知维度和24个子任务，发现当前模型在视觉质量和实际推理深度之间存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 现有的视频生成模型虽然能够通过帧序列进行推理，但缺乏评估其多步规划、算法逻辑和抽象模式外推等核心认知能力的基准。这阻碍了对模型能力的系统理解和改进指导。

Method: 提出Gen-ViRe基准框架，基于认知科学和实际AI应用，将视觉推理分解为6个认知维度（从感知逻辑到抽象规划）和24个子任务，采用多源数据策划、最小提示协议和混合VLM辅助评估。

Result: 实验表明最先进的视频生成系统在视觉质量令人印象深刻，但实际推理深度存在巨大差距，为推进真正的世界模拟器建立了基线和诊断工具。

Conclusion: Gen-ViRe填补了视频模型作为推理器评估的空白，揭示了视觉质量与推理能力的不匹配，为发展真正的世界模拟器提供了系统性的评估框架和改进方向。
...
TL;DR: RSPose提出了一种基于排序的损失函数，解决了热图方法中MSE损失不关注峰值定位、热图不平衡以及评估指标与损失不匹配的问题，在COCO等数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 传统热图人体姿态估计方法存在三个问题：(P1)常用MSE损失平等惩罚所有像素偏差，不专注于锐化和正确定位关节点对应峰值；(P2)热图在空间和类别上不平衡；(P3)评估指标(mAP)与损失函数之间存在差异。

Method: 提出基于排序的损失函数来替代传统的MSE和KL散度损失，使置信度分数与定位质量之间具有更高相关性，从而改善非极大值抑制(NMS)的实例选择和mAP性能。

Result: RSPose在COCO-val上以ViTPose-H达到79.9 AP的SOTA结果，将SimCC Resnet-50提升1.5 AP至73.6 AP，在COCO、CrowdPose、MPII三个数据集上都证明了有效性。

Conclusion: RSPose是首个与评估指标(mAP)对齐的姿态估计损失函数，通过理论分析和实验验证了其优越性，显著提升了人体姿态估计的精度。
...
TL;DR: 本文提出了碰撞声源分割(CS3)新任务，利用音频条件在视觉输入中分割产生碰撞声音的物体，通过弱监督方法和基础模型实现，在两个基准数据集上显著超越基线方法。


<details>
  <summary>Details</summary>
Motivation: 人类擅长多感官感知，能从物体交互的声音中识别物体属性。受此启发，本文旨在让机器也能通过碰撞声音来定位视觉场景中产生该声音的物体，这对理解物理世界和交互具有重要意义。

Method: 提出弱监督的音频条件分割方法，结合CLIP和SAM2基础模型，融入第一人称视角线索（手中的物体）来寻找可能产生碰撞声音的物体，实现声源目标的精确分割。

Result: 在EPIC-CS3和Ego4D-CS3两个基准数据集上，方法在mIoU指标上分别比竞争基线高出3倍和4.7倍，展现出显著的性能优势。

Conclusion: 碰撞声源分割是一个富有挑战性的多模态任务，本文的弱监督方法有效利用音频条件和基础模型，在第一人称视角场景下实现了优异的声源分割效果，为多感官理解提供了新的研究思路。
...
TL;DR: 该研究提出了一个用于膀胱癌复发预测的多序列MRI数据集和H-CNN-ViT模型，通过分层门控注意力机制融合CNN和ViT特征，达到78.6%的AUC性能。


<details>
  <summary>Details</summary>
Motivation: 膀胱癌复发率高(达78%)，传统MRI解读困难，缺乏专门的多序列MRI数据集制约了AI辅助诊断工具的发展。

Method: 构建专门的多序列多模态MRI数据集，提出H-CNN-ViT分层门控注意力多分支模型，独立处理各模态并基于上下文需求选择性地加权全局(ViT)和局部(CNN)特征进行融合。

Result: 在自建数据集上，H-CNN-ViT模型达到78.6%的AUC，超越现有最先进模型，模型代码已公开。

Conclusion: 该研究为膀胱癌复发预测提供了宝贵的数据集基准和有效的AI解决方案，通过创新的特征融合机制显著提升了预测性能。
...
### [23] [Single Tensor Cell Segmentation using Scalar Field Representations](https://arxiv.org/abs/2511.13947)
...
### [26] [CD-DPE: Dual-Prompt Expert Network based on Convolutional Dictionary Feature Decoupling for Multi-Contrast MRI Super-Resolution](https://arxiv.org/abs/2511.14014)
...
### [39] [GCA-ResUNet:Image segmentation in medical images using grouped coordinate attention](https://arxiv.org/abs/2511.14087)
*Jun Ding,Shang Gao*
...
### [40] [BCE3S: Binary Cross-Entropy Based Tripartite Synergistic Learning for Long-tailed Recognition](https://arxiv.org/abs/2511.14097)
...
### [41] [FAPE-IR: Frequency-Aware Planning and Execution Framework for All-in-One Image Restoration](https://arxiv.org/abs/2511.14099)
...
### [42] [Text-Driven Reasoning Video Editing via Reinforcement Learning on Digital Twin Representations](https://arxiv.org/abs/2511.14100)
...
### [43] [RTS-Mono: A Real-Time Self-Supervised Monocular Depth Estimation Method for Real-World Deployment](https://arxiv.org/abs/2511.14107)
...
### [44] [$A^2$GC: $A$symmetric $A$ggregation with Geometric Constraints for Locally Aggregated Descriptors](https://arxiv.org/abs/2511.14109)
...
### [45] [CascadedViT: Cascaded Chunk-FeedForward and Cascaded Group Attention Vision Transformer](https://arxiv.org/abs/2511.14111)
...
### [46] [Coffee: Controllable Diffusion Fine-tuning](https://arxiv.org/abs/2511.14113)
...
### [47] [Multi-view Phase-aware Pedestrian-Vehicle Incident Reasoning Framework with Vision-Language Models](https://arxiv.org/abs/2511.14120)
...
### [48] [Attention Via Convolutional Nearest Neighbors](https://arxiv.org/abs/2511.14137)
...
### [49] [iGaussian: Real-Time Camera Pose Estimation via Feed-Forward 3D Gaussian Splatting Inversion](https://arxiv.org/abs/2511.14149)
...
### [50] [Wave-Former: Through-Occlusion 3D Reconstruction via Wireless Shape Completion](https://arxiv.org/abs/2511.14152)
...
### [51] [Learning Representation and Synergy Invariances: A Povable Framework for Generalized Multimodal Face Anti-Spoofing](https://arxiv.org/abs/2511.14157)
...
### [52] [MVI-Bench: A Comprehensive Benchmark for Evaluating Robustness to Misleading Visual Inputs in LVLMs](https://arxiv.org/abs/2511.14159)
...
### [53] [AdaTok: Adaptive Token Compression with Object-Aware Representations for Efficient Multimodal LLMs](https://arxiv.org/abs/2511.14169)
...
### [54] [O3SLM: Open Weight, Open Data, and Open Vocabulary Sketch-Language Model](https://arxiv.org/abs/2511.14368)
...
### [55] [DoGCLR: Dominance-Game Contrastive Learning Network for Skeleton-Based Action Recognition](https://arxiv.org/abs/2511.14179)
...
### [56] [UniSER: A Foundation Model for Unified Soft Effects Removal](https://arxiv.org/abs/2511.14183)
...
### [57] [GloTok: Global Perspective Tokenizer for Image Reconstruction and Generation](https://arxiv.org/abs/2511.14184)
...
### [58] [PAVE: An End-to-End Dataset for Production Autonomous Vehicle Evaluation](https://arxiv.org/abs/2511.14185)
...
### [60] [Hierarchical Semantic Learning for Multi-Class Aorta Segmentation](https://arxiv.org/abs/2511.14187)
*Pengcheng Shi*
...
### [61] [Online Data Curation for Object Detection via Marginal Contributions to Dataset-level Average Precision](https://arxiv.org/abs/2511.14197)
...
### [68] [SAM-Fed: SAM-Guided Federated Semi-Supervised Learning for Medical Image Segmentation](https://arxiv.org/abs/2511.14302)
...
### [70] [LSP-YOLO: A Lightweight Single-Stage Network for Sitting Posture Recognition on Embedded Devices](https://arxiv.org/abs/2511.14322)
...
### [87] [Improving segmentation of retinal arteries and veins using cardiac signal in doppler holograms](https://arxiv.org/abs/2511.14654)
...
### [116] [XAttn-BMD: Multimodal Deep Learning with Cross-Attention for Femoral Neck Bone Mineral Density Estimation](https://arxiv.org/abs/2511.14604)
...
### [117] [3D-Guided Scalable Flow Matching for Generating Volumetric Tissue Spatial Transcriptomics from Serial Histology](https://arxiv.org/abs/2511.14613)
...
### [118] [Fusing Biomechanical and Spatio-Temporal Features for Fall Prediction: Characterizing and Mitigating the Simulation-to-Reality Gap](https://arxiv.org/abs/2511.14620)
...
### [119] [SparseSurf: Sparse-View 3D Gaussian Splatting for Surface Reconstruction](https://arxiv.org/abs/2511.14633)
...
### [120] [SLAM-AGS: Slide-Label Aware Multi-Task Pretraining Using Adaptive Gradient Surgery in Computational Cytology](https://arxiv.org/abs/2511.14639)
...
### [121] [RepAir: A Framework for Airway Segmentation and Discontinuity Correction in CT](https://arxiv.org/abs/2511.14649)
...
### [123] [HyMAD: A Hybrid Multi-Activity Detection Approach for Border Surveillance and Monitoring](https://arxiv.org/abs/2511.14698)
...
### [125] [FreeSwim: Revisiting Sliding-Window Attention Mechanisms for Training-Free Ultra-High-Resolution Video Generation](https://arxiv.org/abs/2511.14712)
...
### [126] [Diffusion As Self-Distillation: End-to-End Latent Diffusion In One Model](https://arxiv.org/abs/2511.14716)
...
### [127] [Zero-shot Synthetic Video Realism Enhancement via Structure-aware Denoising](https://arxiv.org/abs/2511.14719)
...
### [128] [A Neural Field-Based Approach for View Computation & Data Exploration in 3D Urban Environments](https://arxiv.org/abs/2511.14742)
...
### [129] [Vision Large Language Models Are Good Noise Handlers in Engagement Analysis](https://arxiv.org/abs/2511.14749)
...
### [130] [Co-Me: Confidence-Guided Token Merging for Visual Geometric Transformers](https://arxiv.org/abs/2511.14751)
...
### [131] [UniGen-1.5: Enhancing Image Generation and Editing through Reward Unification in Reinforcement Learning](https://arxiv.org/abs/2511.14760)
...
### [132] [ARC Is a Vision Problem!](https://arxiv.org/abs/2511.14761)
...
### [133] [Signature vs. Substance: Evaluating the Balance of Adversarial Resistance and Linguistic Quality in Watermarking Large Language Models](https://arxiv.org/abs/2511.13722)
*William Guo,Adaku Uchendu,Ana Smith*

Main category: cs.CL

TL;DR: 研究评估了LLM文本水印技术对对抗攻击的脆弱性和对文本质量的影响，发现现有水印技术虽能保持语义但会改变写作风格且容易被攻击规避。


<details>
  <summary>Details</summary>
Motivation: 为了缓解大型语言模型生成文本的潜在危害，研究人员提出了水印技术，但由于这些技术常影响生成文本质量且易受对抗攻击，导致LLM创建者对广泛采用水印技术存在抵制情绪。

Method: 通过比较改写和回译攻击评估水印技术的鲁棒性，并使用语言学指标衡量水印技术保持原始文本质量和写作风格的能力。

Result: 水印技术能保持语义但偏离原始文本的写作风格，且容易受到对抗攻击，特别是回译攻击的威胁。

Conclusion: 现有水印技术在对抗攻击下的脆弱性和对写作风格的改变是阻碍其广泛采用的主要障碍，需要进一步改进以提高鲁棒性和保持文本质量。
...
### [134] [Refine Thought: A Test-Time Inference Method for Embedding Model Reasoning](https://arxiv.org/abs/2511.13726)
*Guangzhi Wang,Kai Li,Yinghao Jiao,Zhi Liu*

Main category: cs.CL

TL;DR: RT（Refine Thought）是一种通过多次前向传播增强文本嵌入模型语义推理能力的测试时推理方法。


<details>
  <summary>Details</summary>
Motivation: 现有的文本嵌入模型在语义推理任务上存在不足，需要一种方法来激活和增强预训练期间学到的语义推理能力。

Method: RT通过运行文本嵌入模型的多次前向传播来获取最终的语义表示，特别适用于仅解码器模型如Qwen3-Embedding-8B。

Result: 在BRIGHT语义推理任务和PJBenchmark1人员岗位匹配基准上取得显著改进，同时在C-MTEB等通用语义理解任务上保持一致性能。

Conclusion: RT通过激活预训练模型中的语义推理能力来提升性能，证明了作为测试时推理方法的有效性。
...
### [135] [Can QE-informed (Re)Translation lead to Error Correction?](https://arxiv.org/abs/2511.13884)
*Govardhan Padmanabhan*

Main category: cs.CL

TL;DR: 本文提出了两种用于机器翻译质量评估指导的段落级错误纠正方法，其中获胜方法通过从多个LLM候选中选择最高质量翻译，在WMT 2025任务中获得第一名。


<details>
  <summary>Details</summary>
Motivation: 尽管联合训练质量评估(QE)与自动后编辑(APE)系统可以提升性能，但APE系统仍存在过度纠正机器翻译输出的问题，导致性能下降。因此需要探索更简单有效的训练无关方法。

Method: 提出两种训练无关方法：1) QE指导的重翻译方法，从不同LLM生成的多个候选中选择最高质量翻译；2) 基于QE解释的LLM错误字符串替换方法。两种方法均采用条件启发式策略最小化编辑次数，最大化增益-编辑比率。

Result: 两种方法分别获得了0.0201和-0.0108的Delta COMET分数，第一种方法在子任务排行榜上获胜，证明了多候选选择策略的有效性。

Conclusion: 训练无关的QE指导错误纠正方法是可行且有效的，特别是多候选选择策略能够有效避免APE系统的过度纠正问题，在实际应用中展现出优越性能。
...
### [136] [What Works for 'Lost-in-the-Middle' in LLMs? A Study on GM-Extract and Mitigations](https://arxiv.org/abs/2511.13900)
*Mihir Gupte,Eshan Dixit,Muhammad Tayyab,Arun Adiththan*

Main category: cs.CL

TL;DR: 该研究提出了GM-Extract基准数据集来评估大语言模型在长距离上下文处理中的"迷失在中间"现象，通过设计双指标评估系统分析了7-8B参数模型的表现，并系统评估了各种缓解方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在利用长距离上下文时能力逐渐减弱的"迷失在中间"现象，对基于检索的LLM应用构成重大挑战，需要在真实应用环境中研究这一现象的影响。

Method: 引入GM-Extract基准数据集，提出包含空间检索能力(文档指标)和语义检索能力(变量提取指标)的双指标评估系统，对7-8B参数模型进行系统性评估，并调研分类缓解方法(黑盒和白盒方法)。

Result: 通过改变上下文窗口中的数据表示方式，发现检索性能发生显著变化；虽然未持续观察到明显的U型曲线，但揭示了模型间的明确性能模式，并与困惑度分数相关联；缓解方法的有效性存在细微差异。

Conclusion: 评估突出了这些策略成功改善性能的场景，以及导致负面影响的一些意外情况，为这些策略在实践环境中的效用提供了全面理解。
...
### [137] [Hint-Augmented Re-ranking: Efficient Product Search using LLM-Based Query Decomposition](https://arxiv.org/abs/2511.13994)
*Yilun Zhu,Nikhita Vedula,Shervin Malmasi*

Main category: cs.CL

TL;DR: 本研究提出了一种基于大语言模型(LLM)的框架，用于解析电商搜索中包含最高级词汇（如"最佳"、"最受欢迎"）的查询，通过提取结构化属性-值提示来理解潜在意图，显著提升搜索效果。


<details>
  <summary>Details</summary>
Motivation: 包含最高级词汇的搜索查询需要在多个维度上比较候选结果，这既需要语言理解能力，也需要领域知识。现有系统难以有效处理这类复杂查询的隐含意图。

Method: 开发了一个基于LLM的框架，提取结构化解释或提示。该方法将查询分解为与检索同时生成的属性-值提示，并有效集成到排序管道中。为解决直接LLM重排序的高延迟问题，还开发了将最高级解释转移到轻量级模型的高效方法。

Result: 该方法在MAP指标上提升10.9个点，在MRR指标上提升5.9个点，显著优于基线方法。同时有效解决了直接LLM重排序的延迟问题。

Conclusion: 研究结果为最高级语义在模型间的表示和转移提供了见解，推进了检索系统中的语言解释能力，同时解决了实际部署中的约束问题。
...
### [138] [HiEAG: Evidence-Augmented Generation for Out-of-Context Misinformation Detection](https://arxiv.org/abs/2511.14027)
*Junjie Wu,Yumeng Fu,Nan Yu,Guohong Fu*

Main category: cs.CL

TL;DR: 本文提出HiEAG框架，通过分层证据增强生成技术改进多模态脱离上下文错误信息检测，超越现有最先进方法。


<details>
  <summary>Details</summary>
Motivation: 现有的脱离上下文(OOC)错误信息检测方法过分强调内部一致性，忽视了图文对与外部证据之间外部一致性的重要性。

Method: 提出HiEAG分层证据增强生成框架，利用多模态大语言模型(MLLMs)的广泛知识。该方法将外部一致性检查分解为包含重排序和重写的完整引擎流程：证据重排序模块使用自动证据选择提示(AESP)获取相关证据，证据重写模块使用自动证据生成提示(AEGP)提高基于MLLM的检测器任务适应性，并通过指令调优实现判断解释。

Result: 在不同基准数据集上的实验结果表明，所提出的HiEAG方法在所有样本的准确率上均超越之前的最先进(SOTA)方法。

Conclusion: HiEAG框架通过有效整合外部证据检查和利用MLLMs能力，显著提升了多模态OOC错误信息检测的性能，为该领域提供了新的解决方案。
...
### [139] [Based on Data Balancing and Model Improvement for Multi-Label Sentiment Classification Performance Enhancement](https://arxiv.org/abs/2511.14073)
*Zijin Su,Huanzhu Lv,Yuren Niu,Yiming Liu*

Main category: cs.CL

TL;DR: 本研究通过构建平衡的多标签情感数据集和增强的分类模型，显著提升了多标签情感分类的性能，解决了现有数据集严重类别不平衡的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的多标签情感分类数据集（如GoEmotions）存在严重的类别不平衡问题，这严重阻碍了模型性能，特别是对代表性不足的情感类别的识别效果很差。

Method: 通过整合原始GoEmotions数据、使用RoBERTa-base-GoEmotions模型从Sentiment140提取的情感标签样本以及人工标注的GPT-4 mini生成文本，构建了包含28个情感类别的平衡数据集。开发了结合FastText词嵌入、CNN局部特征提取、BiLSTM上下文学习和注意力机制的多标签分类模型，采用sigmoid输出层和混合精度训练。

Result: 实验结果表明，与在不平衡数据上训练的模型相比，所提方法在准确率、精确率、召回率、F1分数和AUC等指标上均取得显著提升。

Conclusion: 该研究的数据平衡策略和模型架构设计有效解决了多标签情感分类中的类别不平衡问题，为情感分析领域提供了新的解决方案。
...
### [140] [Stealth Fine-Tuning: Efficiently Breaking Alignment in RVLMs Using Self-Generated CoT](https://arxiv.org/abs/2511.14106)
*Le Yu,Zhengyue Zhao,Yawen Zheng,Yunhao Liu*

Main category: cs.CL

TL;DR: Stealth Fine-Tuning是一种通过段级干扰和自生成数据微调来绕过RVLM安全对齐的低成本高效攻击方法，仅需499样本即可显著提升攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有的推理增强视觉语言模型(RVLMs)通过安全对齐防止有害行为，但其暴露的思维链(CoT)轨迹引入了新的攻击面，现有攻击方法IDEATOR效果有限且成本较高。

Method: 提出Stealth Fine-Tuning方法：通过段级干扰诱使模型生成有害推理轨迹，将自生成输出作为监督微调数据，采用轮转加权损失设计，实现轻量级、分布一致的微调。

Result: 实验表明，仅需499个样本和3小时单A100训练，攻击成功率比IDEATOR提升38.52%，同时保持模型原始表示分布和通用推理能力。

Conclusion: Stealth Fine-Tuning是一种低成本且高效绕过RVLM安全对齐防御的方法，在AdvBench和多个通用基准测试中验证了其有效性，为RVLM安全性研究提供了新的攻击视角。
...
### [141] [Synthetic Clinical Notes for Rare ICD Codes: A Data-Centric Framework for Long-Tail Medical Coding](https://arxiv.org/abs/2511.14112)
...
### [142] [From Graphs to Hypergraphs: Enhancing Aspect-Based Sentiment Analysis via Multi-Level Relational Modeling](https://arxiv.org/abs/2511.14142)
...
### [143] [Applying Relation Extraction and Graph Matching to Answering Multiple Choice Questions](https://arxiv.org/abs/2511.14144)
...
### [144] [Selective Weak-to-Strong Generalization](https://arxiv.org/abs/2511.14166)
...
### [145] [SymLoc: Symbolic Localization of Hallucination across HaluEval and TruthfulQA](https://arxiv.org/abs/2511.14172)
...
### [146] [Harnessing Deep LLM Participation for Robust Entity Linking](https://arxiv.org/abs/2511.14181)
...
### [147] [ArbESC+: Arabic Enhanced Edit Selection System Combination for Grammatical Error Correction Resolving conflict and improving system combination in Arabic GEC](https://arxiv.org/abs/2511.14230)
...
### [148] [MuCPT: Music-related Natural Language Model Continued Pretraining](https://arxiv.org/abs/2511.14245)
...
### [149] [Towards Authentic Movie Dubbing with Retrieve-Augmented Director-Actor Interaction Learning](https://arxiv.org/abs/2511.14249)
...
### [150] [AfriSpeech-MultiBench: A Verticalized Multidomain Multicountry Benchmark Suite for African Accented English ASR](https://arxiv.org/abs/2511.14255)
...
### [151] [Entropy-Guided Reasoning Compression](https://arxiv.org/abs/2511.14258)
...
### [152] [Don't Miss the Forest for the Trees: In-Depth Confidence Estimation for LLMs via Reasoning over the Answer Space](https://arxiv.org/abs/2511.14275)
...
### [153] [AraLingBench A Human-Annotated Benchmark for Evaluating Arabic Linguistic Capabilities of Large Language Models](https://arxiv.org/abs/2511.14295)
...
### [154] [ConInstruct: Evaluating Large Language Models on Conflict Detection and Resolution in Instructions](https://arxiv.org/abs/2511.14342)
...
### [155] [ATLAS: A High-Difficulty, Multidisciplinary Benchmark for Frontier Scientific Reasoning](https://arxiv.org/abs/2511.14366)
...
### [156] [Mitigating Label Length Bias in Large Language Models](https://arxiv.org/abs/2511.14385)
...
### [157] [Unified Defense for Large Language Models against Jailbreak and Fine-Tuning Attacks in Education](https://arxiv.org/abs/2511.14423)
...
### [158] [MedBench v4: A Robust and Scalable Benchmark for Evaluating Chinese Medical Language Models, Multimodal Models, and Intelligent Agents](https://arxiv.org/abs/2511.14439)
...
### [159] [Tell Me: An LLM-powered Mental Well-being Assistant with RAG, Synthetic Dialogue Generation, and Agentic Planning](https://arxiv.org/abs/2511.14445)
...
### [160] [Agent-R1: Training Powerful LLM Agents with End-to-End Reinforcement Learning](https://arxiv.org/abs/2511.14460)
...
### [161] [LiveRAG: A diverse Q&A dataset with varying difficulty level for RAG evaluation](https://arxiv.org/abs/2511.14531)
...
### [162] [Examining the Metrics for Document-Level Claim Extraction in Czech and Slovak](https://arxiv.org/abs/2511.14566)
...
### [163] [A Method for Characterizing Disease Progression from Acute Kidney Injury to Chronic Kidney Disease](https://arxiv.org/abs/2511.14603)
...
### [164] [Bridging Human and Model Perspectives: A Comparative Analysis of Political Bias Detection in News Media Using Large Language Models](https://arxiv.org/abs/2511.14606)
...
### [165] [Enhancing Agentic Autonomous Scientific Discovery with Vision-Language Model Capabilities](https://arxiv.org/abs/2511.14631)
...
### [166] [A Specialized Large Language Model for Clinical Reasoning and Diagnosis in Rare Diseases](https://arxiv.org/abs/2511.14638)
...
### [167] [Graded strength of comparative illusions is explained by Bayesian inference](https://arxiv.org/abs/2511.14642)
...
### [168] [Ground Truth Generation for Multilingual Historical NLP using LLMs](https://arxiv.org/abs/2511.14688)
...
### [169] [Imagine in Space: Exploring the Frontier of Spatial Intelligence and Reasoning Efficiency in Vision Language Models](https://arxiv.org/abs/2511.13782)
*Xiaoxing Lian,Aidong Yang,Jun Zhu,Peng Wang,Yue Zhang*

Main category: cs.AI

TL;DR: 该研究通过构建SpatiaLite基准测试，揭示了当前先进视觉语言模型(VLMs)在空间推理方面存在严重缺陷，主要依赖语言表征而非视觉表征，并提出基于意象驱动的框架来改善空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型和视觉语言模型在逻辑推理、问题解决等方面表现出色，但空间推理作为人类认知的基础组成部分，包括心理旋转、导航和空间关系理解，对当前先进的VLMs来说仍然是一个重大挑战。研究旨在系统性地探测VLMs的空间推理机制。

Method: 研究人员提出了SpatiaLite，一个完全合成的基准测试，能够联合测量空间推理的准确性和效率。通过全面的实验，系统性地测试了当前先进VLMs的空间推理能力，并提出了意象驱动框架(IDF)用于数据合成和训练。

Result: 实验发现：1)先进VLMs主要依赖语言表征进行推理和想象，在需要感知空间关系和3D几何变换的视觉中心任务上存在显著缺陷；2)VLMs在空间推理方面效率严重不足，标记使用量随变换复杂度增加而快速增长；3)提出的意象驱动框架能够隐式构建对空间推理至关重要的内部世界模型。

Conclusion: 该研究通过SpatiaLite基准明确了先进VLMs的空间推理限制和模式，识别了关键缺陷，为未来改进提供了指导。意象驱动框架为构建空间推理所需的内部世界模型提供了新思路，有望推动VLMs在空间理解方面的进步。
...
### [170] [Causal computations in Semi Markovian Structural Causal Models using divide and conquer](https://arxiv.org/abs/2511.13852)
*Anna Rodum Bjøru,Rafael Cabañas,Helge Langseth,Antonio Salmerón*

Main category: cs.AI

TL;DR: 本文研究如何将Bjøru等人提出的分治算法从马尔可夫结构因果模型扩展到半马尔可夫模型，以处理更复杂的混杂关系，并提出了多种替代解决方案进行理论和计算评估。


<details>
  <summary>Details</summary>
Motivation: Bjøru等人提出的分治算法仅适用于马尔可夫模型，但现实中许多因果关系中存在混杂效应，需要使用半马尔可夫模型（外生变量可影响多个内生变量）来表示。因此需要研究如何将该方法扩展到更一般的半马尔可夫模型中。

Method: 通过最小示例说明从马尔可夫扩展到半马尔可夫模型的挑战，提出一套替代解决方案策略，并通过理论分析和计算实验对这些策略进行评估比较。

Result: 通过最小示例揭示了扩展方法的挑战，提出并评估了多种替代策略，为在半马尔可夫结构因果模型中进行反事实概率界限计算提供了可行路径。

Conclusion: 成功将分治算法的思想扩展到半马尔可夫结构因果模型中，虽然面临新的挑战，但提出的替代策略为处理更复杂的混杂关系和计算反事实概率界限提供了有效的解决方案。
...
### [171] [Jailbreaking Large Vision Language Models in Intelligent Transportation Systems](https://arxiv.org/abs/2511.13892)
*Badhan Chandra Das,Md Tasnim Jawad,Md Jueal Mia,M. Hadi Amini,Yanzhao Wu*

Main category: cs.AI

TL;DR: 本文系统分析了智能交通系统中大型视觉语言模型的越狱攻击漏洞，提出了基于图像排版操纵和多轮提示的新型攻击方法，并开发了多层响应过滤防御技术。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在多模态推理和实际应用中表现强大，但极易受到越狱攻击。当集成到智能交通系统时，这些漏洞可能导致严重的安全风险。

Method: 构建包含交通相关有害查询的数据集；提出利用图像排版操纵和多轮提示的新型越狱攻击方法；设计多层响应过滤防御技术；在开源和闭源最先进LVLM上进行广泛实验；使用GPT-4判断和人工验证评估攻击效果和防御性能。

Result: 实验表明，提出的攻击方法对集成在智能交通系统中的LVLM构成严重安全威胁，防御技术能有效阻止模型生成不当响应。与现有越狱技术相比，图像排版操纵和多轮提示攻击风险更为严重。

Conclusion: 研究揭示了LVLM在交通系统应用中的关键安全漏洞，为攻击者和防御者提供了技术参考，强调了加强模型安全性的紧迫性。
...
### [172] [Scene Graph-Guided Generative AI Framework for Synthesizing and Evaluating Industrial Hazard Scenarios](https://arxiv.org/abs/2511.13970)
*Sanjay Acharjee,Abir Khan Ratul,Diego Patino,Md Nazmus Sakib*

Main category: cs.AI

TL;DR: 本研究提出了一种基于场景图引导的生成式AI框架，通过分析OSHA事故报告合成逼真的危险场景图像，并开发了VQA Graph Score评估方法来验证生成图像的真实性和语义保真度。


<details>
  <summary>Details</summary>
Motivation: 获取真实的工作场所危险图像数据极其困难，因为事故发生场景几乎不可能实时捕获，这限制了安全检测视觉模型的训练效果。

Method: 利用GPT-4o分析OSHA事故报告提取结构化危险推理，转换为对象级场景图，然后引导文本到图像扩散模型生成组合准确的危险场景，并提出VQA框架评估生成质量。

Result: 在四种先进生成模型上，提出的VQA Graph Score在基于熵的验证中优于CLIP和BLIP指标，证明其具有更高的判别敏感度。

Conclusion: 该框架有效解决了危险场景数据稀缺问题，为工作场所安全检测模型训练提供了高质量合成数据，VQA Graph Score成为评估生成图像语义保真度的可靠工具。
...
### [173] [Artificial Intelligence Agents in Music Analysis: An Integrative Perspective Based on Two Use Cases](https://arxiv.org/abs/2511.13987)
*Antonio Manuel Martínez-Heredia,Dolores Godrid Rodríguez,Andrés Ortiz García*

Main category: cs.AI

TL;DR: 本文通过整合性回顾和实验验证，展示了AI智能体在音乐分析和教育中的有效性，证明其在模式识别、创作参数化和教育反馈方面优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 旨在弥合AI技术方法与音乐教学应用之间的差距，同时解决透明度、文化偏见和混合评估指标等关键挑战，为教育环境中AI的负责任部署提供指导。

Method: 采用双案例研究方法：(1)在中学教育中使用生成式AI平台培养分析和创作技能；(2)设计用于符号音乐分析的多智能体系统，实现模块化、可扩展和可解释的工作流程。同时综合了从基于规则模型到深度学习、多智能体架构和检索增强生成框架的历史演变。

Result: 实验结果表明，AI智能体能有效增强音乐模式识别、创作参数化和教育反馈，在可解释性和适应性方面优于传统自动化方法。研究发现了透明度、文化偏见和混合评估指标定义方面的关键挑战。

Conclusion: 本研究建立了一个统一框架，连接技术、教学和伦理考量，为计算音乐学和音乐教育中智能体的设计和应用提供了基于证据的指导，强调了在教育环境中负责任部署AI的必要性。
...
### [174] [ALEX:A Light Editing-knowledge Extractor](https://arxiv.org/abs/2511.14018)
*Minghu Wang,Shuliang Zhao,Yuanyuan Zhao,Hongxia Xu*

Main category: cs.AI

TL;DR: 本文提出ALEX框架，一种轻量级知识编辑系统，通过分层内存架构将检索复杂度从O(N)降至O(K+N/C)，显著提升多跳问题回答准确性并减少80%搜索空间。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的静态知识特性难以适应动态变化的信息，现有知识编辑方法在处理复杂多跳推理问题时存在可扩展性和检索效率的瓶颈。

Method: 提出ALEX轻量级知识编辑框架，核心创新包括：1)分层内存架构，将知识更新组织成语义集群；2)推理查询合成(IQS)模块桥接查询与事实的语义差距；3)动态证据裁决(DEA)引擎执行两阶段检索过程。

Result: 在MQUAKE基准测试上，ALEX显著提高了多跳答案准确性(MultiHop-ACC)和推理路径可靠性(HopWise-ACC)，同时将搜索空间减少80%以上。

Conclusion: ALEX为构建可扩展、高效和准确的知识编辑系统提供了有前景的解决方案，通过分层语义组织和智能检索机制有效解决了现有方法的局限性。
...
### [175] [Syn-STARTS: Synthesized START Triage Scenario Generation Framework for Scalable LLM Evaluation](https://arxiv.org/abs/2511.14023)
...
### [177] [Collaborative QA using Interacting LLMs. Impact of Network Structure, Node Capability and Distributed Data](https://arxiv.org/abs/2511.14098)
...
### [178] [APD-Agents: A Large Language Model-Driven Multi-Agents Collaborative Framework for Automated Page Design](https://arxiv.org/abs/2511.14101)
